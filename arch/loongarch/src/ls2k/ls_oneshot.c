/****************************************************************************
 * arch/loongarch/src/ls2k/ls_oneshot.c
 *
...
static int ls_oneshot_handler(int irg_num, void * context, void *arg);

/****************************************************************************
 * Private Data
 ****************************************************************************/

static struct ls_oneshot_s *g_oneshot[CONFIG_LS_ONESHOT_MAXTIMERS];
...
static int ls_oneshot_handler(int irg_num, void * context, void *arg)
{
  struct ls_oneshot_s * oneshot = (struct ls_oneshot_s *) arg;
  ...
  LS_TIM_SETISR(oneshot->tch, NULL, NULL, 0);
  LS_TIM_DISABLEINT(oneshot->tch, GTIM_DIER_UIE);
  LS_TIM_SETMODE(oneshot->tch, LS_TIM_MODE_DISABLED);
  LS_TIM_ACKINT(oneshot->tch, GTIM_SR_UIF);
 ...
 *   timers exceeds CONFIG_LS_ONESHOT_MAXTIMERS.
 *
 ****************************************************************************/

static inline int ls_allocate_handler(struct ls_oneshot_s *oneshot)
{
  ...
  for (i = 0; i < CONFIG_LS_ONESHOT_MAXTIMERS; i++)
    {
                             ...
                             uint16_t resolution)
{
  ...
  LS_TIM_SETCLOCK(oneshot->tch, frequency);
  ...
  return ls_allocate_handler(oneshot);
}

/****************************************************************************
 * Name: ls_oneshot_max_delay
 *
 * Description:
                        ...
                        const struct timespec *ts)
{
  ...
  if (oneshot->running)
    {
      ...
      ls_oneshot_cancel(oneshot, NULL);
    }
  ...
  LS_TIM_SETISR(oneshot->tch, ls_oneshot_handler, oneshot, 0);

  /* Set timer period */

  oneshot->period = (uint32_t)period;
  LS_TIM_SETPERIOD(oneshot->tch, (uint32_t)period);

  /* Start the counter */

  LS_TIM_SETMODE(oneshot->tch, LS_TIM_MODE_PULSE);

  LS_TIM_ACKINT(oneshot->tch, GTIM_SR_UIF);
  LS_TIM_ENABLEINT(oneshot->tch, GTIM_DIER_UIE);
                         ...
                         struct timespec *ts)
{
  ...
  count  = LS_TIM_GETCOUNTER(oneshot->tch);
