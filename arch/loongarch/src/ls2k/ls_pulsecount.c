/****************************************************************************
 * arch/loongarch/src/ls2k/ls_pulsecount.c
 *
...
#include "chip.h"
#include "ls_pulsecount.h"
#include "ls.h"

/* Generalized pulse count support for all LS families */
...
#define PULSECOUNT_TIM1_IRQ LS_IRQ_TIM1
...
struct ls_pulsecount_s
{
  const struct pulsecount_ops_s *ops;
  struct ls_tim_s *timer;
};
...
static uint32_t pulsecount_getreg(struct ls_tim_s *priv, int offset);
static void pulsecount_putreg(struct ls_tim_s *priv, int offset,
                              uint32_t value);
static void pulsecount_modifyreg(struct ls_tim_s *priv, uint32_t offset,
...
static int pulsecount_frequency_update(struct pulsecount_lowerhalf_s *dev,
                                       uint32_t frequency);
static int pulsecount_timer_configure(struct ls_tim_s *priv);
static int pulsecount_channel_configure(struct pulsecount_lowerhalf_s *dev,
                                        uint8_t channel);
static int pulsecount_output_configure(struct ls_tim_s *priv,
                                       struct ls_chan_s *chan);
...
#  ifdef CONFIG_LS_TIM1_PULSECOUNT
...
static struct ls_tim_s g_pulsecount1dev =
{
  .channel =
  {
    .channel = CONFIG_LS_TIM1_PULSECOUNT_CHANNEL,
#if CONFIG_LS_TIM1_PULSECOUNT_CHANNEL == 1
    .out1 =
    {
      .in_use  = 1,
      .pol     = CONFIG_LS_TIM1_PULSECOUNT_POL,
      .idle    = CONFIG_LS_TIM1_PULSECOUNT_IDLE,
      .pincfg  = GPIO_TIM1_CH1OUT,
    },
#elif CONFIG_LS_TIM1_PULSECOUNT_CHANNEL == 2
    .out1 =
    {
      .in_use  = 1,
      .pol     = CONFIG_LS_TIM1_PULSECOUNT_POL,
...
#elif CONFIG_LS_TIM1_PULSECOUNT_CHANNEL == 3
    .out1 =
    {
      .in_use  = 1,
      .pol     = CONFIG_LS_TIM1_PULSECOUNT_POL,
...
#elif CONFIG_LS_TIM1_PULSECOUNT_CHANNEL == 4
    .out1 =
    {
      .in_use  = 1,
      .pol     = CONFIG_LS_TIM1_PULSECOUNT_POL,
  ...
  .t_dts       = CONFIG_LS_TIM1_PULSECOUNT_TDTS,
  .irq         = PULSECOUNT_TIM1_IRQ,
  .base        = LS_TIM1_BASE,
  .pclk        = TIMCLK_TIM1,
};

#endif /* CONFIG_LS_TIM1_PULSECOUNT */
...
#ifdef CONFIG_LS_TIM1_PULSECOUNT
static struct ls_pulsecount_s g_pulsecount1lower =
{
  .ops = &g_pulsecountops,
...
static void pulsecount_dumpregs(struct pulsecount_lowerhalf_s *dev,
                                const char *msg)
{
  struct ls_tim_s *priv = (struct ls_tim_s *)dev;

  _info("%s:\n", msg);
  _info("  CR1: %04x CR2:  %04x SMCR:  %04x DIER:  %04x\n",
          pulsecount_getreg(priv, LS_GTIM_CR1_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CR2_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_SMCR_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_DIER_OFFSET));

  _info("   SR: %04x EGR:  %04x CCMR1: %04x CCMR2: %04x\n",
          pulsecount_getreg(priv, LS_GTIM_SR_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_EGR_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CCMR1_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CCMR2_OFFSET));

  _info(" CCER: %04x CNT:  %04x PSC:   %04x ARR:   %04x\n",
          pulsecount_getreg(priv, LS_GTIM_CCER_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CNT_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_PSC_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_ARR_OFFSET));

  if (priv->timid == 1)
    {
      _info("  RCR: %04x BDTR: %04x\n",
          pulsecount_getreg(priv, LS_ATIM_RCR_OFFSET),
          pulsecount_getreg(priv, LS_ATIM_BDTR_OFFSET));
    }

  _info(" CCR1: %04x CCR2: %04x CCR3:  %04x CCR4:  %04x\n",
          pulsecount_getreg(priv, LS_GTIM_CCR1_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CCR2_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CCR3_OFFSET),
          pulsecount_getreg(priv, LS_GTIM_CCR4_OFFSET));

  _info("  DCR: %04x DMAR: %04x\n",
      pulsecount_getreg(priv, LS_GTIM_DCR_OFFSET),
      pulsecount_getreg(priv, LS_GTIM_DMAR_OFFSET));
}
#endif
                                  ...
                                  uint8_t channel, ub16_t duty)
{
  ...
  reload = pulsecount_getreg(priv, LS_GTIM_ARR_OFFSET);
                                       ...
                                       uint32_t frequency)
{
  ...
  pulsecount_putreg(priv, LS_GTIM_ARR_OFFSET, reload);
  pulsecount_putreg(priv, LS_GTIM_PSC_OFFSET, (uint16_t)(prescaler - 1));
...
static int pulsecount_timer_configure(struct ls_tim_s *priv)
{
  ...
  cr1 = pulsecount_getreg(priv, LS_GTIM_CR1_OFFSET);
  ...
  pulsecount_putreg(priv, LS_GTIM_CR1_OFFSET, cr1);
                                       ...
                                       struct ls_chan_s *chan)
{
  ...
  cr2  = pulsecount_getreg(priv, LS_GTIM_CR2_OFFSET);
  ccer = pulsecount_getreg(priv, LS_GTIM_CCER_OFFSET);
  ...
  pulsecount_modifyreg(priv, LS_GTIM_CR2_OFFSET, 0, cr2);
  pulsecount_modifyreg(priv, LS_GTIM_CCER_OFFSET, 0, ccer);
 ...
 * Input Parameters:
 *   dev     - A reference to the lower half driver state structure
 *   outputs - outputs to set (look at enum ls_pulsecount_chan_e)
                                     ...
                                     uint16_t outputs, bool state)
{
  ...
  pulsecount_putreg(priv, LS_GTIM_CCER_OFFSET, ccer);
                                  ...
                                  bool enable)
{
  ...
  if (enable)
    {
      pulsecount_modifyreg(priv, LS_ATIM_BDTR_OFFSET, 0, ATIM_BDTR_MOE);
    }
  else
    {
      pulsecount_modifyreg(priv, LS_ATIM_BDTR_OFFSET, ATIM_BDTR_MOE, 0);
    }
}
...
static uint16_t
pulsecount_outputs_from_channels(struct ls_tim_s *priv, uint8_t selected)
{
  uint16_t outputs = 0;
  uint8_t  channel;
...
static int pulsecount_configure(struct pulsecount_lowerhalf_s *dev)
{
  ...
  pulsecount_modifyreg(priv, LS_GTIM_CR1_OFFSET, GTIM_CR1_CEN, 0);
                            ...
                            const struct pulsecount_info_s *info)
{
  ...
  pulsecount_putreg(priv, LS_GTIM_DIER_OFFSET, 0);
  pulsecount_putreg(priv, LS_GTIM_SR_OFFSET, 0);
  ...
  if (info->count > 0)
    {
      ...
      pulsecount_putreg(priv, LS_ATIM_RCR_OFFSET, priv->prev - 1);

      /* Generate an update event to reload the prescaler.  This should
       * preload the RCR into active repetition counter.
       */

      pulsecount_putreg(priv, LS_GTIM_EGR_OFFSET, GTIM_EGR_UG);
      ...
      pulsecount_putreg(priv, LS_ATIM_RCR_OFFSET, priv->curr - 1);
    }
  ...
  /* Otherwise, just clear the repetition counter */
  ...
  else
    {
      ...
      pulsecount_putreg(priv, LS_ATIM_RCR_OFFSET, 0);
  ...
  if (info->count > 0)
    {
      ...
      pulsecount_putreg(priv, LS_GTIM_SR_OFFSET, 0);
      pulsecount_putreg(priv, LS_GTIM_DIER_OFFSET, GTIM_DIER_UIE);

      /* Enable the timer */

      pulsecount_modifyreg(priv, LS_GTIM_CR1_OFFSET, 0, GTIM_CR1_CEN);
...
static int pulsecount_interrupt(struct pulsecount_lowerhalf_s *dev)
{
  ...
  regval = pulsecount_getreg(priv, LS_ATIM_SR_OFFSET);
  DEBUGASSERT((regval & ATIM_SR_UIF) != 0);

  /* Clear the UIF interrupt bit */

  pulsecount_putreg(priv, LS_ATIM_SR_OFFSET, (regval & ~ATIM_SR_UIF));
...
static int pulsecount_ll_stop(struct pulsecount_lowerhalf_s *dev)
{
  ...
  pulsecount_putreg(priv, LS_GTIM_CCR1_OFFSET, 0);
  pulsecount_putreg(priv, LS_GTIM_CCR2_OFFSET, 0);
  pulsecount_putreg(priv, LS_GTIM_CCR3_OFFSET, 0);
  pulsecount_putreg(priv, LS_GTIM_CCR4_OFFSET, 0);
...
static int pulsecount_setup(struct pulsecount_lowerhalf_s *dev)
{
  struct ls_pulsecount_s *pulse = (struct ls_pulsecount_s *)dev;
  int ret;
...
{
  struct ls_pulsecount_s *pulse = (struct ls_pulsecount_s *)dev;
  struct ls_tim_s *priv = pulse->timer;
...
{
  struct ls_pulsecount_s *lower = NULL;
