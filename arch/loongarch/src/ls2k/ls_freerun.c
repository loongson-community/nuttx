/****************************************************************************
 * arch/loongarch/src/ls2k/ls_freerun.c
 *
...
static int ls_freerun_handler(int irq, void *context, void *arg)
{
  struct ls_freerun_s *freerun = (struct ls_freerun_s *) arg;

  DEBUGASSERT(freerun != NULL && freerun->overflow < UINT32_MAX);
  freerun->overflow++;

  LS_TIM_ACKINT(freerun->tch, GTIM_SR_UIF);
  return OK;
}
                          ...
                          uint16_t resolution)
{
  ...
  LS_TIM_SETCLOCK(freerun->tch, frequency);
  ...
  freerun->chan         = chan;
  freerun->width        = LS_TIM_GETWIDTH(freerun->tch);
  ...
  LS_TIM_SETISR(freerun->tch, ls_freerun_handler, freerun, 0);
#endif

  /* Set timer period */

  LS_TIM_SETPERIOD(freerun->tch,
                      (uint32_t)((1ull << freerun->width) - 1));

  /* Start the counter */

  LS_TIM_SETMODE(freerun->tch, LS_TIM_MODE_UP);

#ifndef CONFIG_CLOCK_TIMEKEEPING
  LS_TIM_ACKINT(freerun->tch, GTIM_SR_UIF);
  LS_TIM_ENABLEINT(freerun->tch, GTIM_DIER_UIE);
#endif
                       ...
                       struct timespec *ts)
{
   ...
   * careful here because  ls_tc_getpending() will reset the pending
  ...
  counter  = LS_TIM_GETCOUNTER(freerun->tch);
  pending  = LS_TIM_CHECKINT(freerun->tch, 0);
  verify   = LS_TIM_GETCOUNTER(freerun->tch);
...
int ls_freerun_counter(struct ls_freerun_s *freerun, uint64_t *counter)
{
  *counter = LS_TIM_GETCOUNTER(freerun->tch);
  return OK;
}
...
int ls_freerun_uninitialize(struct ls_freerun_s *freerun)
{
  ...
  LS_TIM_DISABLEINT(freerun->tch, GTIM_DIER_UIE);
  LS_TIM_SETMODE(freerun->tch, LS_TIM_MODE_DISABLED);
  LS_TIM_SETISR(freerun->tch, NULL, NULL, 0);

  /* Free the timer */

  ls_tim_deinit(freerun->tch);
  freerun->tch = NULL;
